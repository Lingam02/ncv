...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
  ...
  $currentDate = date('Y-m-d'); // Format the date (Year-Month-Day)
  $currentTime = date('H:i:s'); // Format the time (Hour:Minute:Second)
  
  // echo "Current date in India: $currentDate<br>";
  // echo "Current time in India: $currentTime";
  ...
  if ($locationid != "") {
    ...
    // First table INSERT
    $query1 = "INSERT INTO `tbl_opening`(`date`,`time`,`loc_id`, `loc_name`, `itm_type`, `tbl_type`, `col_id`, `col_nam`, `wght`, `qty`,box_id, box_no) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
    $stmt1 = mysqli_prepare($con, $query1);

    if ($stmt1) {
      for ($key = 0; $key < count($wept_colours_id); $key++) {
        if ($wept_colours_id[$key] !== "" && $wghts[$key] > 0) {
          mysqli_stmt_bind_param($stmt1, "ssssssssddss",$currentDate,$currentTime, $locationid, $location, $itm_type, $tbl_type1, $wept_colours_id[$key], $wept_colours[$key], $wghts[$key], $qtys[$key],$boxid[$key],$box[$key]);
    ...
    // Second table INSERT
    $query2 = "INSERT INTO `tbl_opening`(`date`,`time`,`loc_id`, `loc_name`, `itm_type`, `tbl_type`, `itm_id`, `itm_nam`, `wght`, `qty`,`box_id`,`box_no`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
    $stmt2 = mysqli_prepare($con, $query2);

    if ($stmt2) {
      for ($key = 0; $key < count($zari_id); $key++) {
        if ($zari_id[$key] !== "" && $wghts[$key] > 0) {
          mysqli_stmt_bind_param($stmt2, "ssssssssddss",$currentDate,$currentTime, $locationid, $location, $itm_type, $tbl_type2, $zari_id[$key], $zari[$key], $wghts2[$key], $qtys2[$key],$boxid2[$key],$box2[$key]);
...
<body>
  ...
  <div class="container" id="pirnstore">
    <h4 class="text-center mt-2 text-warning">Opening Balance of Pirn Store</h4>
          ...
          <table id="wept_tbl">
            ...
            <tbody id="tbody_ps_weft">
              <tr>
                ...
                <td>
                  <input type="number" class="form-control" value="0" name="qtys[]" onclick="this.select()" onkeydown="limitDecimal(this, 3)" onkeyup="limitDecimal(this, 3)" oninput="limitDecimal(this, 3)">
                </td>
              </tr>
          ...
          <table id="zari_tbl">
            ...
            <tbody id="tbody_ps_zari">
              <tr>
                ...
                <td>
                  <input type="number" class="form-control" value="0" name="wghts2[]" onclick="this.select()"onkeydown="limitDecimal(this, 3)" onkeyup="limitDecimal(this, 3)" oninput="limitDecimal(this, 3)">
                </td>
                <td>
                  <input type="number" class="form-control" value="0" name="qtys2[]" onclick="this.select()"onkeydown="limitDecimal(this, 3)" onkeyup="limitDecimal(this, 3)" oninput="limitDecimal(this, 3)">
                </td>
              </tr>
