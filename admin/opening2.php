  ...
  if ($locationid != "") {
    ...
    if ($stmt1) {
      for ($key = 0; $key < count($wept_colours_id); $key++) {
        if ($wept_colours_id[$key] !== "" && $bobin_name[$key] !== "" && $wghts[$key] > 0) {
    ...
    if ($stmt2) {
      for ($key = 0; $key < count($zari_id); $key++) {
        if ($zari_id[$key] !== "" && $bobin_name2[$key] !== "" && $wghts2[$key] >0) {
...
<body>
  ...
  <div class="container" id="bobinstore">
    <h4 class="text-center mt-2 text-danger">Opening Balance of Bobin Store </h4>
