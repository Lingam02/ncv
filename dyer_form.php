...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($wght)) {
        ...
        if ($wght != "") {
            $sql = "INSERT INTO dyer_hd (tnx_id, tnx_type, itm_type, dyer_nam, dyer_id,loc_nam, loc_id,date, time, tag_no, descript, wght, itm_nam,loom_id) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($con, $sql);
            // $new_warp_no = $warp_no[$key].'/'.($key+1);
            mysqli_stmt_bind_param($stmt, "sssssssssssdss", $tnx_id, $tnx_type, $itm_type, $dyer_nam, $dyer_id, $loc_nam, $loc_id, $save_date, $save_time, $tag_no, $descript, $wght, $itm_nam, $loom_id);
...
<body>
                ...
                <div class="container-fluid">
                    ...
                    <div class="container">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-6">
                                <label for="iss_desc">Select Colour</label>

                                <!-- <input type="text" class="form-control" id="iss_desc" name="iss_desc" placeholder="" onkeydown="handleEnterKey(event, 'iss_wght')">                                    -->
                                <div class="d-flex ">
                                    <input type="radio" class="ms-3" id="single" name="saree_color" placeholder="" onkeydown="handleEnterKey(event, 'iss_wght')">
                                    <label for="single">Single Colour</label>


                                    <input type="radio" class="ms-3" id="multi" name="saree_color" placeholder="" onkeydown="handleEnterKey(event, 'iss_wght')">
                            ...
                            <div class="col-sm-3">
                                <div id="single_color" style="display:none">
                                    <table>
                                        <!-- <thead>
                                            <tr>
                                                <th>Color</th>
                                ...
                                <div id="multi_color" style="display:none">
                                    <table>
                                        <!-- <thead>
                                            <tr>
                                                <th>multi Color</th>
                        ...
                        <div id="entry_table_div2" class="mt-3">
                            <table id="entry_table2">
                                <thead>
                                    ...
                                    <tr>
                                        <th style="width:150px">Warp No</th>
                                        <th>Type</th>
                                        <!--  -->
                                        <th>Yard</th>
                                        <th>No of Saree</th>
    ...
    <script>
        ...
        function handleRadioButtonChange() {
            ...
            var multiColorRadioButton = document.getElementById('multi');
            var singleclorDiv = document.getElementById('single_color');
            var multiclorDiv = document.getElementById('multi_color');

            if (singleRadioButton.checked) {
                singleclorDiv.style.display = 'block';
                multiclorDiv.style.display = 'none';
            } else if (multiColorRadioButton.checked) {
                singleclorDiv.style.display = 'none';
                multiclorDiv.style.display = 'block';
            }
        }

        // Attach event listener to radio buttons
        document.getElementById('single').addEventListener('change', handleRadioButtonChange);
        document.getElementById('multi').addEventListener('change', handleRadioButtonChange);

        // Call the function initially to set initial display
